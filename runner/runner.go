...
type job struct {
    id       int64
    expectAt time.Time
    call     func(*job) (afterContinue bool, nextInterval time.Duration)
    caller   string
    // cancel   bool
}
...
func (r *Runner) executeJobs() (int64, time.Time) {
    ...
    for i := 0; i <= len(r.jobs); i++ {
        ...
        //     continue
        // }
        if dur := j.expectAt.Sub(now); dur < precisionInterval {
    ...
    for _, j := range exeJobs {
        ...
        var call = j.call
        var callAt = time.Now()
        go func(call func(callJob *job) (bool, time.Duration), wait chan struct{}) {
            nextContinue, nextDuration = call(j)
            wait <- struct{}{}
        }(call, wait)
        ...
        if nextContinue && nextDuration > precisionInterval {
            j.expectAt = now.Add(nextDuration)
            repeatJobs = append(repeatJobs, j)
        }
        count++
    }
...
func (r *Runner) addJob(j *job) (id int64) {
    r.jobsMu.Lock()
    if r.seqJobID == math.MaxInt64 {
        r.seqJobID = 0
    }
    r.seqJobID++
    j.id = r.seqJobID
...
func (r *Runner) addInstantJob(call func(), over chan struct{}, caller string) int64 {
    runAt := time.Now()
    return r.addJob(&job{
        expectAt: runAt,
        call: func(self *job) (afterContinue bool, nextInterval time.Duration) {
            call()
            if over != nil {
                close(over)
            }
...
func (r *Runner) addTimerJob(interval time.Duration, repeats int, startNow bool, call func(), over chan struct{}, caller string) int64 {
    counter := 0
    expectCount := repeats
    at := time.Now()
    if !startNow {
...
func (r *Runner) cancelJob(id int64) {
    r.jobsMu.Lock()
...
func (r *Runner) TimerRunWithID(interval time.Duration, repeat int, call func()) int64 {
    caller := getCaller(3)
    addJobResult := make(chan int64)
    if interval <= 0 {
        go func() {
            id := r.addInstantJob(call, nil, caller)
            addJobResult <- id
            r.tick()
        }()
    } else {
        go func() {
            id := r.addTimerJob(interval, repeat, false, call, nil, caller)
            addJobResult <- id
        }()
    }
    return <-addJobResult
...
func (r *Runner) CancelTimer(id int64) {
    go func() {
        r.cancelJob(id)
        r.tick()
    }()
